...
const isStaticPath = (path: string[]): boolean => staticFilesSet.has(path[0]) || staticFilesSet.has(path[1])
...
const getNextStaticUrl = (url: URL, path: string[]): string => {
    // If static file is at root (path[0]), use it directly
    // If static file is prefixed (path[1]), remove the prefix
    const nextPath = staticFilesSet.has(path[0]) ? path : path.slice(1)
...
const ADS_TXT_CONTENT = `# KO Digital Websites! #
ownerdomain=kodigital.io
...
export const onRequest: PagesFunction = async (context) => {
    ...
    try {
        ...
        // Serve ads.txt directly from function to bypass cached static file
        if (path[0] === 'ads.txt' || path[1] === 'ads.txt') {
